...
Label(root, font=("Comic Sans MS", 16), text='Mad Libs Generator').place(x=80, y=0)
...
def first_madlib(win):
        def finish_madlib(tl: Toplevel, animal, parent, month, path, movement, num):
            text = f'''
    There was once a {animal}. He was always getting told off. One day while his 
    {parent} was sitting in the garden in {month}, 
    he sneaked out. He did not mean to go far but he saw a glittery thing on the 
    {path} and {movement} over to it. He found out it was gold and became rich because he had {num} pieces of gold.'''
            tl.geometry(newGeometry='375x550')
            Label(tl, text='Your Story:', font=("Times", 13, 'bold'), background='Gold',
                  wraplength=tl.winfo_width()).place(x=130, y=310)
...
def second_madlib(win):
  def finish_madlib(tl: Toplevel, boy, cloth_article, creature, adj, village, exclamation, fav_thing):
    text = f'''
On one dark, stormy night, {boy} came along. He was wearing nothing but a {cloth_article}. He went into the woods and there he found a {creature}! It was {adj}. In fright he ran into a nearby village called {village} screaming, "{exclamation}!" The footsteps of the {creature} behind him became louder and louder. The villagers screamed, "It's a {adj} {creature}!" 
...
Then it went back into the woods and waited for its next victim to emerge.'''
    tl.geometry(newGeometry='400x700')
